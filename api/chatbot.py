...
# --------------------------------------------------------------------------
# Multi-language UI translation (Hybrid approach)
# --------------------------------------------------------------------------

# Supported regional target languages + the native-script name we ask
# Gemini to translate into. Keep this tight — every new entry multiplies
# the free-tier daily quota we consume.
_TRANSLATE_TARGETS = {
    'pa': 'Pakistani Shahmukhi Punjabi (Perso-Arabic script, as spoken in Lahore/Faisalabad)',
    'ps': 'Pashto (Perso-Arabic script with پ ټ ډ ښ characters, as spoken in Khyber Pakhtunkhwa)',
    'sd': 'Sindhi (Perso-Arabic script with ٻ ڀ ڃ ڄ characters, as spoken in Sindh)',
    'skr': 'Saraiki (extended Shahmukhi, as spoken in southern Punjab)',
    'bal': 'Balochi (Perso-Arabic script, as spoken in Pakistani Balochistan)',
}

# Hard cap so a runaway client can't ask us to translate 10,000 keys in
# a single request and blow through the Gemini RPD quota.
_MAX_TRANSLATE_ENTRIES = 80
_MAX_TRANSLATE_CHARS_PER_ENTRY = 400
...
class TranslateBatchView(APIView):
    ...
    Body: {
      "target_lang": "pa|ps|sd|skr|bal",
      "entries": {"home": "Home", "flock_name": "Flock Name", ...}
    }
    Response: {"translations": {"home": "...", ...}}

    Called by the Flutter LanguageProvider as a background best-effort
    request. The client caches results in SharedPreferences so we only
    pay the Gemini round-trip once per (key, language) pair per device.
    """
    permission_classes = [IsAuthenticated]

    def post(self, request):
        target_lang = (request.data.get('target_lang') or '').strip().lower()
        if target_lang not in _TRANSLATE_TARGETS:
            return Response(
        ...
        if len(entries) > _MAX_TRANSLATE_ENTRIES:
            return Response(
                {'error': 'too_many_entries'},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Sanitise input — drop non-string values and cap individual
        # string length so a malicious client can't send megabytes of
        # text through the translate endpoint.
        clean = {}
        for k, v in entries.items():
            ...
            if not key or not val:
                continue
            if len(val) > _MAX_TRANSLATE_CHARS_PER_ENTRY:
                val = val[:_MAX_TRANSLATE_CHARS_PER_ENTRY]
            clean[key] = val
        if not clean:
            return Response({'translations': {}})

        if not _ensure_client():
            logger.error('Translate: Gemini not configured: %s', _GENAI_ERROR)
            return Response(
        ...
        prompt_lines = [
            'You are translating a mobile app UI into '
            f'{_TRANSLATE_TARGETS[target_lang]}.',
            'Rules:',
            '- Produce natural, simple wording farmers recognise.',
            '- Keep disease names that farmers already know (Ranikhet, Coccidiosis) in native script if common.',
            '- Do NOT add extra words, explanation, or markdown.',
            '- Preserve placeholder tokens like {name}, %s, $1 verbatim.',
            '- Return STRICT JSON only. Schema: {"translations": {"<key>": "<native translation>"}}.',
            '- Every input key must appear in the output with its translation (no keys dropped).',
            '',
            'Input JSON:',
        ...
        try:
            response = _CLIENT.models.generate_content(
                ...
                contents=[
                    _GENAI_TYPES.Content(
                        role='user',
                        parts=[_GENAI_TYPES.Part.from_text(text=prompt)],
                    ),
                ],
                config=_GENAI_TYPES.GenerateContentConfig(
                    temperature=0.2,
            ...
            if not isinstance(translations, dict):
                return Response({'translations': {}})
            # Keep only string values for keys we asked about — defends
            # against the model inventing extra keys or returning nested
            # objects.
            cleaned = {
                k: str(v).strip()
                for k, v in translations.items()
                if isinstance(k, str) and isinstance(v, (str, int, float)) and k in clean
            }
            return Response({'translations': cleaned})
        except Exception as e:  # noqa: BLE001
            logger.exception('Gemini translate-batch call failed')
            return Response(
                {
                    'error': 'upstream_error',
                    'error_type': type(e).__name__,
